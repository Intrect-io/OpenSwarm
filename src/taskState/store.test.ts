...
describe('task state store', () => {
  ...
  it('reloads after a same-size state-file replacement within one mtime tick', async () => {
    // Titles must be equal length so the on-disk JSON stays the same byte size.
    const fixture = fileURLToPath(new URL('./storeClaimProcess.fixture.ts', import.meta.url));
    await new Promise<void>((resolve, reject) => {
      const child = spawn(
        process.execPath,
        ['--import', 'tsx', fixture, stateFile, 'SWAP-SRC', '0', '--same-size-replace'],
        { stdio: 'pipe' },
      );
      let stderr = '';
      child.stderr.on('data', (chunk) => { stderr += String(chunk); });
  ...
  it('does not complete a parent when a child is Canceled — that is not Done', () => {
    upsertTaskState('PARENT-C', {
      childIssueIds: ['CHILD-C1', 'CHILD-C2'],
      execution: { status: 'decomposed', retryCount: 0 },
      linearState: 'In Progress',
      updatedAt: new Date().toISOString(),
    });
    upsertTaskState('CHILD-C1', {
      parentIssueId: 'PARENT-C',
      execution: { status: 'done', retryCount: 0 },
      linearState: 'Done',
      updatedAt: new Date().toISOString(),
    });
    upsertTaskState('CHILD-C2', {
      parentIssueId: 'PARENT-C',
      execution: { status: 'backlog', retryCount: 0 },
      linearState: 'Canceled',
      updatedAt: new Date().toISOString(),
    ...
    expect(completeParentIfChildrenDone('CHILD-C1')).toBeNull();
    expect(getTaskState('PARENT-C')?.execution.status).toBe('decomposed');
  });
