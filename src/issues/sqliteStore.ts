...
export class SqliteIssueStore implements IIssueStore {
  ...
  private migrate(): void {
    ...
    `);
    // Enforce unique Linear-to-local issue mappings: concurrent inbound sync must
    // not create multiple local issues for one Linear issue. Existing duplicates
    // are collapsed first (keep the oldest row) so the unique index can be built.
    const uniqueMappingMigration = this.db.prepare('SELECT 1 FROM schema_migrations WHERE name = ?').get('issues_linear_unique_v1');
    if (!uniqueMappingMigration) {
      this.db.transaction(() => {
        this.db.prepare(`
          DELETE FROM issues
            ...
            AND id NOT IN (
              SELECT id FROM (
                SELECT id, ROW_NUMBER() OVER (
                  PARTITION BY linear_id ORDER BY created_at ASC, id ASC
                ) AS rn FROM issues WHERE linear_id IS NOT NULL
              ) WHERE rn = 1
            )
        `).run();
        this.db.prepare(`
          ...
          WHERE linear_identifier IS NOT NULL
            AND id NOT IN (
              SELECT id FROM (
                SELECT id, ROW_NUMBER() OVER (
                  PARTITION BY linear_identifier ORDER BY created_at ASC, id ASC
                ) AS rn FROM issues WHERE linear_identifier IS NOT NULL
              ) WHERE rn = 1
            )
        `).run();
        this.db.prepare('DROP INDEX IF EXISTS idx_issues_linear').run();
        this.db.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_issues_linear_id ON issues(linear_id) WHERE linear_id IS NOT NULL').run();
        this.db.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_issues_linear_identifier ON issues(linear_identifier) WHERE linear_identifier IS NOT NULL').run();
        this.db.prepare('INSERT INTO schema_migrations (name, applied_at) VALUES (?, ?)')
          .run('issues_linear_unique_v1', new Date().toISOString());
      })();
    }
