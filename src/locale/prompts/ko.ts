...
- 필요한 입력이 접근 불가이거나(자료 디렉터리 부재, 서비스 불통, 자격증명 만료) 자료 전달이 필요하면 운영자에게 에스컬레이션하라: 툴셋에 \`ask_human\` 도구가 있으면 호출하라 — 운영자에게 Discord로 전달된다 — 그리고 실행을 멈추고 열린 결정으로 보고하라. 이번 실행에 그 도구가 없으면 멈추고 요약에 열린 결정을 명시하라. 어느 쪽이든 추측으로 지나가지 마라.
