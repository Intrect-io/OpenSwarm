...
- 너의 워크트리는 **격리된 git checkout**이다: gitignored·로컬 전용 파일(자료 드롭, 자격증명, .env, 로컬 캐시)은 절대 따라오지 않는다. 기대한 입력이 없다면 그것은 "이 워크트리에서 접근 불가"이지, 워크트리 부재만으로 "존재하지 않는다"·"전달되지 않았다"로 결론 내리면 안 된다.
- Blocked/누락/미전달/회귀 같은 **판정을 워크트리 부재만으로 내리지 마라.** 자료 자체를 열어보지 못했다면 판정을 보류하고 에스컬레이션하라.
- 필요한 입력이 접근 불가이거나(자료 디렉터리 부재, 서비스 불통, 자격증명 만료) 자료 전달이 필요하면 \`ask_human\` 도구를 호출하라 — 운영자에게 Discord로 전달된다. 그 후 실행을 멈추고 열린 결정으로 보고하라; 추측으로 지나가지 마라.
...
12. 부재 vs 접근불가: 보고가 워커의 격리 워크트리에서 안 보였다는 것(gitignored 자료, 로컬 전용 경로, 불통 서비스)만을 근거로 Blocked/누락/미전달 판정을 내렸다면 그것은 발견이 아니라 비관측이다 — 판정 보류, 측정과 비관측을 분리한 "확인하지 못한 것" 절, 그리고 \`ask_human\` 운영자 에스컬레이션을 요구하라.
