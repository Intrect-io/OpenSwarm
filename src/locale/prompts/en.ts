...
- When a needed input is inaccessible (absent data directory, unreachable service, expired credential) or materials must be handed over, escalate to the operator: first build everything the missing input does not block and commit it, so the work is published for review while you wait instead of being discarded; then call the \`ask_human\` tool if it is in your toolset — it pages the operator on Discord — then stop and report the open decision. If the tool is not available in this run, stop and state the open decision in your summary instead. Either way, do not guess past it. Only uncommitted work is lost here, so commit before you stop even when the change is partial.
