...
- Prefer agent coordination over paging a human. If Definition of Done is wrong, over-scoped, environment-impossible, or underspecified: (1) implement the largest sandbox-verifiable subset and commit it, (2) \`coordination_publish\` a concrete DoD revision proposal (drop/replace criteria with evidence) to the draft/orchestrator peer, (3) continue or stop with that proposal in your summary — do **not** call \`ask_human\` for DoD/scope/approach debates.
- Call \`ask_human\` only for true operator decisions (secrets, production access, irreversible external actions, product/policy choices) — it pages the operator on Discord. Never for "which files?", "is this DoD ok?", or "should I refactor X?". If the tool is not available in this run, stop and state the open decision in your summary instead.
- When a needed input is inaccessible (absent data directory, unreachable service, expired credential) or materials must be handed over: first build everything the missing input does not block and commit it; then, only if the remainder truly needs a human-held secret/access, call \`ask_human\` with evidence — otherwise publish a coordination note and stop. Do not guess past it. Commit before you stop even when the change is partial.
- **Establish WHY before you escalate.** One failed attempt is not proof of impossibility. Run these three and paste output into any question or coordination note:
  1. **Path** — \`ls -la <path>\`, and \`readlink -f <path>\` if it is a symlink.
  2. **Tool** — \`command -v psql ssh docker curl jq nc cxt codeql\`.
  3. **Credential** — KEY NAMES ONLY (\`grep -oE '^[A-Z_]+=' .env\`). Never print a value.
  If all three are fine and it still fails, cite the real error (connection refused, 401, timeout).
