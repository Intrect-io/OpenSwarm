...
- When a needed input is inaccessible (absent data directory, unreachable service, expired credential) or materials must be handed over, call the \`ask_human\` tool — it pages the operator on Discord. Then stop and report the open decision; do not guess past it.
...
12. Absence vs inaccessibility: if the report grounds a verdict (Blocked / missing / not delivered) solely in something being absent from the worker's isolated worktree (gitignored data, local-only paths, unreachable services), that is a non-observation, not a finding — require the verdict to be withheld, a "Could not verify" section separating measurements from non-observations, and an \`ask_human\` escalation to the operator instead.
