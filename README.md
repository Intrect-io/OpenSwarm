...
The `openrouter` adapter runs OpenSwarm's own agentic tool loop (read/search/edit/bash with verification guards), enables ZDR (`data_collection: deny`) for non-OpenAI models, and applies Anthropic prompt caching automatically. Requests sort providers by throughput and prefer low latency (nitro-speed, without the `:nitro` suffix that unlocks paid priority tiers), then cap price at the cheapest endpoint × `OPENROUTER_PRICE_MULTIPLIER` (default `3`). Pin a backend with `OPENROUTER_PROVIDER_ONLY`. Local backends are auto-detected on standard ports (Ollama `:11434`, LM Studio `:1234`); use `lmstudio` for a dedicated LM Studio endpoint (`LMSTUDIO_BASE_URL`, default `http://localhost:1234`).
